# See also https://pychao.com/2019/11/02/optimize-tensorflow-keras-models-with-l-bfgs-from-tensorflow-probability/


# Copyright 2016 The TensorFlow Authors. All Rights Reserved.
# TensorFlow 2.0 campatibility by Serhiy Kozak. [partial support]
#
...
class ExternalOptimizerInterface(object):
    ...
    def __init__(self,
        ...
        Args:
            tf_eval_func: tensorflow function to optimize
            loss: A scalar `Tensor` to be minimized.
            var_list: `list` of `Variable` objects to update to minimize
        ...
        """
        self._tf_eval_func = tf_eval_func
#    self._loss = loss
...
#    ]
#
#    loss_grads = _compute_gradients(tape, loss, self._vars)
#    equalities_grads = [
#        _compute_gradients(tape, equality, self._vars)
#        for equality in self._equalities
#    ]
#    inequalities_grads = [
#        _compute_gradients(tape, inequality, self._vars)
...
#    ]
        ...
        self._packing_shapes = [var.shape for var in self._vars]
    ...
    def minimize(self,
        ...
        Args:
            extra_params: A feed dict to be passed to calls to `session.run`.
        ...
        loss_grad_func = self._make_eval_func(self._tf_eval_func,
                                                                                    extra_params, fetches, loss_callback)

#    # Construct equality constraint functions and associated gradients.
#    equality_funcs = self._make_eval_funcs(self._tf_eval_func, extra_params,
#                                           fetches)
#    equality_grad_funcs = self._make_eval_funcs(self._tf_eval_func, extra_params, fetches)
#
#    # Construct inequality constraint functions and associated gradients.
#    inequality_funcs = self._make_eval_funcs(self._tf_eval_func,
#                                             extra_params, fetches)
#    inequality_grad_funcs = self._make_eval_funcs(self._tf_eval_func, extra_params, fetches)
#
#    # Get initial value from TF session.
#    initial_packed_var_val = self._tf_eval_func(self._vars, parameters=extra_params)

        # Perform minimization.
        res = self._minimize(
                loss_grad_func=loss_grad_func,
                equality_funcs=None,
                equality_grad_funcs=None,
                inequality_funcs=None,
                inequality_grad_funcs=None,
...
#    session.run(
#        self._var_updates,
#        extra_params=dict(zip(self._update_placeholders, var_vals)),
#        **run_kwargs)
        
        return res #, self._vars

    def _minimize(self, loss_grad_func, equality_funcs,
    ...
    def _make_eval_func(self, eval_fn, extra_params, fetches,
                                            callback=None):
        """Construct a function that evaluates a `Tensor` or list of `Tensor`s."""

        def eval_func(x):
            """Function to evaluate a `Tensor`."""
#      augmented_extra_params = {
#          var: x[packing_slice].reshape(_get_shape_tuple(var))
#          for var, packing_slice in zip(self._vars, self._packing_slices)
#      }
#      augmented_extra_params.update(extra_params)

            # assign new value
            self._packed_var = tf.Variable(x)
         
            with tf.GradientTape() as tape:
                # monitor packed variable
                tape.watch(self._packed_var)
                
                # unpack
                self._vars = self.unpack(self._packed_var)
                
                # call tensor objective function
                self._outputs = self._tf_eval_func(self._vars, *extra_params)
                obj = self._extract_objective(self._outputs) # self._outputs[0]

            # compute gradients
            grad = _compute_gradients(tape, obj, self._packed_var)
            
            if callable(callback):
                callback(self._iter, self._outputs, self._vars)
    ...
    def moments_jacobian(self, extra_params, obj_idx=1, parallel_iterations=64, experimental_use_pfor=False):
        return self.func_jacobian(self._tf_eval_func, extra_params, obj_idx=obj_idx, parallel_iterations=parallel_iterations, experimental_use_pfor=experimental_use_pfor)
...
#      
#      # call tensor objective function
#      outputs = self._tf_eval_func(self._vars, *extra_params)
#      obj = outputs[1]
#
#    # compute gradients
#    jac = tape.jacobian(obj, self._packed_var, experimental_use_pfor=False)
#    del tape
#    
#    return jac

    # compute a jacobian for any given function
    def func_jacobian(self, func, extra_params, obj_idx=0, parallel_iterations=64, experimental_use_pfor=False):
        with tf.GradientTape(persistent=True) as tape:
            # monitor packed variable
            tape.watch(self._packed_var)
            ...
            # call tensor objective function
            outputs = func(self._vars, *extra_params)
            obj = self._extract_objective(outputs, obj_idx)

        # compute gradients
        jac = tape.jacobian(obj, self._packed_var, parallel_iterations=parallel_iterations, experimental_use_pfor=experimental_use_pfor)
        del tape
        
        return jac, outputs
    
    
    def unpack(self, packed_var):
        # unpack
        unpacked_vars = [
                tf.reshape(packed_var[packing_slice], packing_shape) for packing_slice, packing_shape in zip(self._packing_slices, self._packing_shapes)
        ]
        return unpacked_vars
        
    def _extract_objective(self, outputs, obj_idx=0):
        if obj_idx or isinstance(outputs, list) or isinstance(outputs, tuple):
            obj = outputs[obj_idx]
        else:
            obj = outputs
...
#                       callback=None):
#    return [
#        self._make_eval_func(tensor, eval_fn, extra_params, fetches, callback)
...
class ScipyOptimizerInterface(ExternalOptimizerInterface):
    ...
    def _minimize(self, loss_grad_func, equality_funcs,
        ...
        def loss_func_wrapper(x):
            ...
            return loss.numpy() # , gradient.numpy() #.astype('float64')

        def grad_func_wrapper(x):
            # SciPy's L-BFGS-B Fortran implementation requires gradients as doubles.
            loss, gradient = loss_grad_func(x)
            return gradient.numpy() #.astype('float64')
...
#      constraints.append({'type': 'ineq', 'fun': func, 'jac': grad_func})
        ...
        minimize_args = [loss_func_wrapper, self._packed_var]
        minimize_kwargs = {
...
def _compute_gradients(tape, tensor, var):
    return tape.gradient(tensor, var)
#  # tf.gradients sometimes returns `None` when it should return 0.
#  return [
#      grad.numpy() if grad is not None else array_ops.zeros_like(var)
